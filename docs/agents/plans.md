...
ブランチの作成元、命名、PR の向きに関する共通ルールは `branch-rules.md` に従う。
...
- parent issue ブランチ:
- child issue ブランチ:
- child issue の PR の向き:
- parent issue ブランチの最終 PR の向き:
...
ブランチ名、作成元、PR の向きは `branch-rules.md` に従う。

ExecPlan には、作業対象の parent issue / child issue に応じて、実際に使用するブランチ名と PR の向きを記載する。

## 並列作業時のルール

child issue を並列で対応する場合のブランチ運用は `branch-rules.md` に従う。

ExecPlan の編集範囲も `branch-rules.md` の並列作業時のルールに従う。

## 更新ルール

ExecPlan と実装内容がずれた場合は、実装内容に合わせて ExecPlan も更新する。

ただし、並列作業中の ExecPlan 編集範囲は `branch-rules.md` に従う。
...
- 必要なテストが通る
- 必要なドキュメントが更新されている
- PR の向きが `branch-rules.md` に従っている
