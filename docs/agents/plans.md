...
- parent issue:
- parent issue ブランチ: `issue-xxx`
- この child issue のブランチ: `issue-xxx`
- PR の向き: `issue-xxx` → `issue-xxx`
...
ブランチ名は、parent issue / child issue のどちらも `issue-xxx` に統一する。
`xxx` には対応する issue 番号を入れる。
親子関係はブランチ名では表現せず、GitHub Issue の親子関係、PR の向き、ExecPlan の記載で管理する。
