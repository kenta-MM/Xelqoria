...
- Window / EventLoop / Input / Dialog / Clipboard / Cursor の OS 依存は Platform 抽象を経由し、OS 固有実装は Platform.Win32 / Platform.Mac へ閉じ込める
...
- Platform.Win32 / Platform.Mac 以外で Window / EventLoop / Input / Dialog / Clipboard / Cursor の OS ネイティブ API を直接使用しない
- Editor.UI で OS ネイティブ UI シェルを実装しない
- 依存逆転の #include を作らない
- using で名前空間を省略しない
- グローバル変数を定義しない
...
- ゲームロジックは Game
- OS 抽象は Platform
- Window / EventLoop / Input / Dialog / Clipboard / Cursor の OS 固有 API は Platform.Win32 / Platform.Mac
- OS 非依存の Editor UI 入力補助 / SceneView 表示境界は Editor.UI
...
- Window / EventLoop / Input / Dialog / Clipboard / Cursor の OS 固有 API は Platform.Win32 / Platform.Mac に閉じているか
- Editor の OS ネイティブ UI シェルが Editor.UI へ漏れていないか
...
1. 依存方向維持
2. OS 抽象は Platform
3. Window / EventLoop / Input / Dialog / Clipboard / Cursor の OS 固有実装は Platform.Win32 / Platform.Mac
4. 低レベル GPU 抽象は RHI
5. 高レベル描画は Graphics
